...
이 판단은 지금까지 엔지니어의 판단에 의존해왔습니다. WaferGuard는 이 운영 판단을 **RAG 기반 멀티 에이전트**로 자동화합니다.
...
- **Multi-agent Delegation** : 탐지 모델의 오탐으로 판단될 경우나, 데이터 분포의 변화가 클 경우 자동으로 MLOps 에이전트로 위임되어 탐지 모델의 재학습 여부를 판단합니다.
